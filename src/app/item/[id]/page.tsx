...
import { ArrowLeft, Gavel, User, MessageCircle, Send } from "lucide-react";
...
export default function ItemDetail() {
  ...
  async function handleContactSeller(e: React.FormEvent) {
    e.preventDefault();
    if (!user || !item) { router.push("/auth"); return; }
          ...
          <div className="space-y-6">
            ...
            {item.is_auction && auction ? (
              ...
              <div className="space-y-4">
                ...
                {user && user.id !== item.seller_id && !messageSent ? (
                  <div className="space-y-2">
                    ...
                    {showMessageBox && (
                      <form onSubmit={handleContactSeller} className="space-y-2">
                        <textarea
                          value={messageText}
                          onChange={(e) => setMessageText(e.target.value)}
                          className="tavern-input w-full h-20 resize-none text-sm"
                          placeholder={`Hi! I'm interested in "${item.title}"...`}
                        />
                        {messageError && (
                          <p className="text-tavern-red text-xs">⚠️ {messageError}</p>
                        )}
                        <button
                          type="submit"
                          disabled={messageSending || !messageText.trim()}
                          className="tavern-btn-secondary w-full flex items-center justify-center gap-1 text-sm py-2"
                        >
                          {messageSending ? "⏳ Sending..." : <><Send size={14} /> Send Message</>}
                        </button>
                      </form>
                    )}
                  </div>
                  ...
                  <div className="bg-tavern-green/20 text-tavern-green p-3 rounded-lg text-center text-sm">
                    ✅ Message sent! The seller will reply in your{" "}
                    <Link href="/messages" className="underline font-medium">Messages</Link>.
                  </div>
                ) : (
                  <button
                    ...
                    className="tavern-btn w-full flex items-center justify-center gap-2"
                  >
                    <MessageCircle size={18} /> Sign in to Contact
                  </button>
                )}
              </div>
            )}
